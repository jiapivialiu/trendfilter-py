...
Eigen::MatrixXd computePtemp(const Eigen::MatrixXd& A, const Eigen::MatrixXd& P) {
  int k = P.rows();
  Eigen::MatrixXd P_copy = P;  // Make a copy since we need to modify it
  Eigen::MatrixXd temp = A.row(0) * P_copy;
  Eigen::MatrixXd var = A.row(0) * temp.transpose();

  // in-place block replacement starting from the last component:
  P_copy.block(1, 1, k - 1, k - 1).reverse() = P_copy.block(0, 0, k - 1, k - 1).reverse();
  P_copy(0, 0) = var(0, 0);
  temp.conservativeResize(1, k - 1);  // drop the last component
  P_copy.block(0, 1, 1, k - 1) = temp;
  P_copy.block(1, 0, k - 1, 1) = temp.transpose();
  return P_copy;
}
...
configure_denseD_test(const Eigen::VectorXd& x, int k) {
    ...
    return std::make_tuple(dk_mat, s_seq, denseD);
}
