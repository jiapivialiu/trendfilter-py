...
def example_multiple_lambdas():
    ...
    if hasattr(tf, 'degrees_of_freedom_'):
        print(f"Degrees of freedom range: {tf.degrees_of_freedom_.min():.1f} - {tf.degrees_of_freedom_.max():.1f}")
    ...
    if tf.coef_.ndim > 1:
        for i, idx in enumerate([0, len(lambda_values)//4, len(lambda_values)//2, -1]):
            plt.plot(x, tf.coef_[:, idx], '--', alpha=0.8, 
...
def example_cross_validation():
    ...
    plt.axvline(cv_tf.best_lambda_, color='red', linestyle='--', 
...
def main():
    ...
    try:
        from trendfilter import get_backend_info
